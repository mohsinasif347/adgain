...
export default function AdminDashboard() {
  ...
  const handleLogout = async () => {
    ...
    if (!error) {
      window.location.href = '/'; // Full page reload to clear all states
    } else {
    ...
    <div className="min-h-screen bg-white dark:bg-[#0b1120] text-slate-900 dark:text-white pb-20 overflow-x-hidden transition-colors duration-500">
      ...
      <header className="px-6 pt-8 pb-4 flex justify-between items-center relative z-10">
        <div className="text-left">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-widest text-left">Admin Control</p>
          </div>
          <h2 className="text-xl font-black italic text-left">Overview Panel</h2>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="p-2.5 bg-slate-100 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/10 text-slate-500 dark:text-slate-400 active:scale-95 transition-all"
          >
            <LogOut size={20} />
          </button>
        </div>
      ...
      >
        ...
        <motion.div variants={itemVariants} className="bg-gradient-to-br from-blue-700 to-indigo-900 dark:from-blue-600 dark:to-indigo-950 rounded-[2.5rem] p-8 shadow-2xl shadow-blue-500/20 border border-white/10 relative overflow-hidden group">
          ...
          <div className="text-left">
            ...
            <div className="flex items-baseline gap-2 mt-2">
              <h1 className="text-5xl font-black tracking-tighter text-white italic text-left">
          ...
          <div className="mt-8 flex gap-3">
             <div className="flex-1 bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/10 text-left">
                <p className="text-[10px] text-blue-100 uppercase font-bold text-left">Ads Watched</p>
                <p className="text-lg font-black mt-1 text-white text-left"><AnimatedCounter to={data?.stats?.total_ads || 0} /></p>
             </div>
             <div className="flex-1 bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/10 text-left">
                <p className="text-[10px] text-blue-100 uppercase font-bold text-left">Total Users</p>
                <p className="text-lg font-black mt-1 text-white text-left"><AnimatedCounter to={data?.stats?.total_users || 0} /></p>
             </div>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-2 gap-4">
          <div className="bg-slate-50 dark:bg-[#161e2d] border border-black/5 dark:border-white/5 p-5 rounded-3xl shadow-sm text-left">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase text-left">Pending Payouts</p>
            <p className="text-xl font-black mt-1 text-orange-600 dark:text-orange-500 italic text-left">{data?.stats?.pending_withdrawals || 0}</p>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-[#161e2d] border border-black/5 dark:border-white/5 p-5 rounded-3xl shadow-sm text-left">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase text-left">System Status</p>
            <p className="text-xl font-black mt-1 text-emerald-600 dark:text-emerald-500 italic text-left">Active</p>
          </div>
        </motion.div>
            ...
            {data?.recent_withdrawals?.map((w: any) => (
              <div key={w.id} className="bg-slate-50 dark:bg-[#161e2d] border border-black/5 dark:border-white/5 p-4 rounded-2xl flex justify-between items-center group transition-all hover:shadow-lg">
                <div className="flex items-center gap-3 text-left">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100 text-left">{w.full_name}</p>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight text-left">{w.payment_method}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black text-slate-900 dark:text-white italic text-right">{w.amount} <span className="text-[10px] text-yellow-600 dark:text-yellow-500">COINS</span></p>
        ...
        {/* New Users List */}
        <motion.div variants={itemVariants} className="space-y-4">
          <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 px-1 text-left">New Registrations</h4>
            ...
            {data?.recent_users?.map((u: any) => (
               <div key={u.id} className="bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-black/5 dark:border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-3 text-left">
                    ...
                    <p className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate max-w-[140px] text-left">{u.full_name}</p>
                  </div>
                  <p className="text-xs font-black text-emerald-600 dark:text-emerald-400 italic text-right">+{u.balance}</p>
               </div>
            ))}
          </div>
        </motion.div>
